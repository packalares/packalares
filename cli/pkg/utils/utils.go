...
func EncryptPassword(plainText string) string {
	return utils.MD5(plainText + GetPasswordSalt())
}

const (
	passwordSaltEnvVar  = "PACKALARES_PASSWORD_SALT"
	passwordSaltFile    = "/etc/packalares/password-salt"
	passwordSaltLength  = 32
)

// GetPasswordSalt returns the password salt, reading it from the environment
// variable PACKALARES_PASSWORD_SALT if set, otherwise from the file
// /etc/packalares/password-salt. If the file does not exist, a random 32-char
// salt is generated and persisted to that file.
func GetPasswordSalt() string {
	if salt := os.Getenv(passwordSaltEnvVar); salt != "" {
		return salt
	}

	data, err := os.ReadFile(passwordSaltFile)
	if err == nil {
	...
	salt, err := GeneratePassword(passwordSaltLength)
	if err != nil {
		// Fallback: use crypto/rand hex encoding
		b := make([]byte, passwordSaltLength/2)
		_, _ = crypto.Read(b)
		salt = fmt.Sprintf("%x", b)
	}

	// Persist the salt to file
	dir := "/etc/packalares"
	if err := os.MkdirAll(dir, 0700); err == nil {
		_ = os.WriteFile(passwordSaltFile, []byte(salt+"\n"), 0600)
	}
