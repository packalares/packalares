...
func (o *activateUserOptions) AddFlags(cmd *cobra.Command) {
	...
	cmd.Flags().StringVar(&o.BflUrl, "bfl", fmt.Sprintf("http://127.0.0.1:%d", config.WizardPort), fmt.Sprintf("Bfl URL (e.g., https://example.com, default: http://127.0.0.1:%d)", config.WizardPort))
	cmd.Flags().StringVar(&o.VaultUrl, "vault", fmt.Sprintf("http://127.0.0.1:%d", config.WizardPort), fmt.Sprintf("Vault URL (e.g., https://example.com, default: http://127.0.0.1:%d)", config.WizardPort))
	cmd.Flags().StringVarP(&o.Password, "password", "p", "", "OS password for authentication, required for activation")
	cmd.Flags().StringVar(&o.Location, "location", envStr("PACKALARES_TIMEZONE", "UTC"), "Timezone location (default: UTC, env: PACKALARES_TIMEZONE)")
	cmd.Flags().StringVar(&o.Language, "language", envStr("PACKALARES_LANGUAGE", "en-US"), "System language (default: en-US, env: PACKALARES_LANGUAGE)")
