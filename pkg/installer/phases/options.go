...
// writeConfigYAML writes /etc/packalares/config.yaml from installer options.
// This must run before any phase that uses config.*() functions.
func writeConfigYAML(opts *InstallOptions) error {
	content := fmt.Sprintf(`# Packalares Configuration — generated by installer
system:
  hostname: packalares
  domain: %s
  timezone: UTC
...
`, opts.Domain, opts.Username,
		opts.TailscaleAuthKey != "", opts.TailscaleAuthKey)
