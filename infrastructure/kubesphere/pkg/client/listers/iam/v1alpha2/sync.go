...
type SyncLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.Sync, err error)
	// Get retrieves the Sync from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha2.Sync, error)
	SyncListerExpansion
}
...
// List lists all Syncs in the indexer.
func (s *syncLister) List(selector labels.Selector) (ret []*v1alpha2.Sync, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.Sync))
	})
	return ret, err
}

// Get retrieves the Sync from the index for a given name.
func (s *syncLister) Get(name string) (*v1alpha2.Sync, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("sync"), name)
	}
	return obj.(*v1alpha2.Sync), nil
}
