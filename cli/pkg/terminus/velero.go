...
func (c *CreateBackupLocation) Execute(runtime connector.Runtime) error {
	...
	var ns = "os-framework"
	var provider = "packalares"
	var storage = "packalares-backup"
...
func (i *InstallVeleroPlugin) Execute(runtime connector.Runtime) error {
	...
	cmd = fmt.Sprintf("%s install --no-default-backup-location --namespace %s --image %s --use-volume-snapshots=false --no-secret --plugins %s --velero-pod-cpu-request=10m --velero-pod-cpu-limit=200m --node-agent-pod-cpu-request=10m --node-agent-pod-cpu-limit=200m --wait --wait-minute 30 %s", velero, ns, images.Ref("velero", veleroVersion), images.Ref("velero-plugin-for-terminus", veleroPluginVersion), args)
...
func (v *PatchVelero) Execute(runtime connector.Runtime) error {
	...
	var ns = "os-framework"
	var patch = `[{"op":"replace","path":"/spec/template/spec/volumes","value": [{"name":"plugins","emptyDir":{}},{"name":"scratch","emptyDir":{}},{"name":"packalares-backup","hostPath":{"path":"/olares/rootfs/k8s-backup", "type":"DirectoryOrCreate"}}]},{"op": "replace", "path": "/spec/template/spec/containers/0/volumeMounts", "value": [{"name":"plugins","mountPath":"/plugins"},{"name":"scratch","mountPath":"/scratch"},{"mountPath":"/data","name":"packalares-backup"}]},{"op": "replace", "path": "/spec/template/spec/containers/0/securityContext", "value": {"privileged": true, "runAsNonRoot": false, "runAsUser": 0}}]`
